...
## Contents
  ...
  - [2. Customize the parsing](#2-customize-the-parsing)
  - [3. Customize the analysis](#3-customize-the-analysis)
    - [3.1. Number of components and scaling](#31-number-of-components-and-scaling)
    - [3.2. Analysis methods](#32-analysis-methods)
    - [3.3. Connection between blocks](#33-connection-between-blocks)
      - [3.3.1. Loading a connection file](#331-loading-a-connection-file)
      - [3.3.2. Superblock](#332-superblock)
      - [3.3.3. Supervised analysis](#333-supervised-analysis)
    - [3.4. Other R/SGCCA parameters](#34-other-rsgcca-parameters)
      - [3.4.1. Shrinkage parameter (Tau)](#341-shrinkage-parameter-tau)
      - [3.4.2. Sparsity coefficient](#342-sparsity-coefficient)
      - [3.4.3. Scheme function (advanced users)](#343-scheme-function-advanced-users)
  - [4. Customize the graphics](#4-customize-the-graphics)
    - [4.1. Color the samples](#41-color-the-samples)
    - [4.2. Display names](#42-display-names)
    - [4.3. Components (for the x/y-axis)](#43-components-for-the-xy-axis)
    - [4.4. Block (for the x/y-axis)](#44-block-for-the-xy-axis)
    - [4.5. Number of top variables](#45-number-of-top-variables)
  - [5. Visualize the plot](#5-visualize-the-plot)
    - [5.1. Connection between blocks](#51-connection-between-blocks)
    - [5.2. Average variance explained (AVE)](#52-average-variance-explained-ave)
    - [5.3. Samples](#53-samples)
    - [5.4. Corcircle](#54-corcircle)
    - [5.5. Top variables](#55-top-variables)
...
The analyse parameters are all set by default (**Fig. 5**) and the user could directly click on the ```Execute``` button. To directly visualize the outputs, see the [last section](#5-visualize-the-plot).
...
The downloaded folder contains a design matrix (```connection.tsv```; **Fig. 6**) corresponding to the relationship between each block: 1 if two blocks are connected and 0 otherwise. The file should not have column and row names. It is a symmetric matrix with the same dimension as the number of blocks. It allows to add *a priori* information of correlation hypothesis between the blocks. It will not be taken in account with a superblock (see next section). After disabling the ```use a superblock``` option, ```load the design matrix``` and observe the result on the plots. The ```connection.tsv``` file contains 1 in all non-diagonal cells and makes the assumption that all the blocks are related.
...
A variable could be used to color the points according to a response. For this, load the ``` political_system.tsv``` file (**Fig. 8**) in the corresponding ```color the individual plot[...]``` box to update the plot. The expected format is : 
...
By default, plots are shown with the ```superblock``` (i.e., the concatenation of all blocs; see [section 3.3.2](#332-superblock)) to visualize all the blocs together. If this option is disabled, by default, the last blocks in the drop-down menu ```block``` is used (option ```0```). Choose another block (e.g., ```1``` for agriculture) to update the plots with your selection. For the ```individual``` plot, a ```block for the y-axis``` could also be selected.
...
```design.pdf``` summarizes the connection between each block: a link corresponds to a "1" value, in the matrix connection file (**Fig. 9**; see [section 3.3.1.](#331-loading-a-connection-file)). For each block:
- "P" is the number of variables
- "N" is the number of lines (here, each block has the same number of line)
- "tau" is the shrinkage parameter and "sparsity" is the sparsity coefficient (see the [3.4.1 & 3.4.2 sections](#34-other-rsgcca-parameters). The tau parameter could be shown for each component if the optimal option is selected

![Fig 9](https://raw.githubusercontent.com/rgcca-factory/RGCCA/master/img/design.png)
...
![Fig 10](https://raw.githubusercontent.com/rgcca-factory/RGCCA/master/img/ave.png)
...
![Fig 11](https://raw.githubusercontent.com/rgcca-factory/RGCCA/master/img/individuals.png)
...
![Fig 12](https://raw.githubusercontent.com/rgcca-factory/RGCCA/master/img/corcircle.png)
...
![Fig 13](https://raw.githubusercontent.com/rgcca-factory/RGCCA/master/img/top_variables.png)
